...
def main():
    ...
    train_dataset = CarDamageDataset(
        train_images, train_masks, train_transform, config.damage_color, config.color_tolerance
    )
    val_dataset = CarDamageDataset(
        val_images, val_masks, val_transform, config.damage_color, config.color_tolerance
    )
    ...
    if getattr(config, 'push_to_hub', False):
        if getattr(config, 'hf_token', None):
            print(f"Pushing to Hugging Face Hub: {config.model_name}")
            try:
                trainer.push_to_hub(
